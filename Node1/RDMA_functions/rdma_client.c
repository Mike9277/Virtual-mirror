...
struct client_context
{
    ...
    struct timeval start_time, end_time;  // Timing fields
};
...
static void send_next_chunk(struct rdma_cm_id *id)
{
    ...
    if (size > 0) {
        ...
        gettimeofday(&ctx->end_time, NULL);  // Stop timing
        long seconds = ctx->end_time.tv_sec - ctx->start_time.tv_sec;
        long useconds = ctx->end_time.tv_usec - ctx->start_time.tv_usec;
        long total_microseconds = (seconds * 1000000) + useconds;
        printf("Total time taken: %ld microseconds\n", total_microseconds);
        rc_disconnect(id);  // Disconnect after sending the entire file
    }
}
...
static void send_file_name(struct rdma_cm_id *id)
{
    ...
    gettimeofday(&ctx->start_time, NULL);  // Start timing
...
static void on_pre_conn(struct rdma_cm_id *id)
{
    ...
    if (posix_memalign((void **)&ctx->buffer, sysconf(_SC_PAGESIZE), BUFFER_SIZE)) {
        perror("posix_memalign failed for buffer");
        exit(EXIT_FAILURE);
    }
    TEST_Z(ctx->buffer_mr = ibv_reg_mr(rc_get_pd(), ctx->buffer, BUFFER_SIZE, 0));

    if (posix_memalign((void **)&ctx->msg, sysconf(_SC_PAGESIZE), sizeof(*ctx->msg))) {
        perror("posix_memalign failed for msg");
        exit(EXIT_FAILURE);
    }
