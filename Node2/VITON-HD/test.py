...
def test(opt, seg, gmm, alias):
    ...
    with torch.no_grad():
        for i, inputs in enumerate(test_loader.data_loader):
            ...
            # Part 1: Segmentation generation (low-res)
            ...
            seg_input = torch.cat((cm_down, c_masked_down, parse_agnostic_down, pose_down,
                                   gen_noise(cm_down.size()).to(device)), dim=1)

            parse_pred_down = seg(seg_input)
            parse_pred_down = gauss(parse_pred_down)  # blur in low-res
            parse_pred = F.interpolate(parse_pred_down, size=(opt.load_height, opt.load_width),
                                       mode='bilinear').argmax(dim=1)[:, None]

            # Mapping semantico
            parse_old = torch.zeros(parse_pred.size(0), 13, opt.load_height, opt.load_width, device=device)
            parse_old.scatter_(1, parse_pred, 1.0)
            labels_tensor = [[0], [2, 4, 7, 8, 9, 10, 11], [3], [1], [5], [6], [12]]
            parse = torch.stack([parse_old[:, idxs].sum(1) for idxs in labels_tensor], dim=1)
            ...
            _, warped_grid = gmm(torch.cat((parse_cloth_gmm, pose_gmm, agnostic_gmm), dim=1), c_gmm)
            ...
            save_images(output, unpaired_names, os.path.join(opt.save_dir))
