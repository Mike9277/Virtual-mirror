...
class VITONDataset(data.Dataset):
    def __init__(self, opt):
        ...
        # single input mode
        self.img_names = [osp.basename(opt.person)]
        self.c_names = {'unpaired': [osp.basename(opt.cloth)]}
        self.person_path = opt.person
        self.cloth_path = opt.cloth
    ...
    def __getitem__(self, index):
        ...
        c_name = {}
        c = {'unpaired': Image.open(self.cloth_path).convert('RGB')}
        cm = {}

        for key in self.c_names:
            c_name[key] = self.c_names[key][index]
            c[key] = Image.open(osp.join(self.data_path, 'cloth', c_name[key])).convert('RGB')
            c[key] = transforms.Resize((self.load_height, self.load_width), interpolation=InterpolationMode.BILINEAR)(c[key])
            cm[key] = Image.open(osp.join(self.data_path, 'cloth-mask', c_name[key]))
            cm[key] = transforms.Resize((self.load_height, self.load_width), interpolation=InterpolationMode.NEAREST)(cm[key])
        ...
        # load person image
        img = Image.open(self.person_path).convert('RGB')
